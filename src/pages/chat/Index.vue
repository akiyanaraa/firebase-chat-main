        ...
        <div class="relative bg-repeat bg-[url('assets/bg.jpg')] bg-opacity-25 rounded h-full flex flex-col flex-grow">
          ...
          <div class="sticky bottom-0 left-0 w-full px-2 py-3 bg-blue-200">
            <div class="flex items-center">
              <input ref="input" v-model="message" class="input-text rounded rounded-r-none w-full" @keyup.enter="send"/>
              <button @click="send" class="btn btn-info rounded-l-none ml-2">Send</button>
            </div>
          </div>
        </div>
